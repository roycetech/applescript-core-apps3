...
build-counter:
	...
	$(call _build-script,libraries/counter-plist/dec-counter-daily)
	$(call _build-script,libraries/counter-plist/counter)
...
build-cliclick:
	...
	@if command -v cliclick >/dev/null 2>&1; then \
		osascript libraries/cliclick/setup-cliclick-cli.applescript; \
		./scripts/build-lib.sh libraries/cliclick/cliclick; \
...
build-jira:
	...
	$(call _build-script,libraries/jira/jira)
...
build-json:
	...
	$(call _build-script,libraries/json/json)
...
build-log4as:
	...
	$(call _build-script,libraries/log4as/log4as)
...
# 	plutil -replace 'LoggerSpeechAndTrackingInstance' -string 'dec-logger-log4as' ~/applescript-core/config-lib-factory.plist
	...
	cp -n libraries/log4as/log4as.plist.template ~/applescript-core/log4as.plist || true
...
PROCESS_SCRIPTS = $(wildcard libraries/process/*.applescript)
build-process:
...
# build-process:
# 	# $(SUDO) ./scripts/build-lib.sh libraries/process/process
# 	yes y | ./scripts/build-lib.sh libraries/process/process
...
build-redis:
	...
	$(call _build-script,libraries/redis/redis)
	@echo "Build redis scripts completed\n"

build-redis-terminal:
	@echo "Building Redis terminal scripts..."
	$(call _build-script,libraries/redis/dec-terminal-prompt-redis)
...
build-security-cli:
	...
	$(call _build-script,libraries/security-cli/security-cli)
	@echo "Build Security CLI scripts completed\n"


install-timed-cache:
	cp -n plist.template ~/applescript-core/timed-cache.plist || true
	$(call _build-script,libraries/timed-cache-plist/timed-cache-plist)
