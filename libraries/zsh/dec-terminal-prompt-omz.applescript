(*
	...
	@Build:
		./scripts/build-lib.sh libraries/zsh/dec-terminal-prompt-omz
	...
	script TerminalPromptOmzDecorator
		...
		*)
		on isShellPrompt()
			...
			set originalScroll to terminal's getScrollPosition()
			-- logger's debugf("originalScroll: {}", originalScroll)
			
			set didChangeScroll to false
			if originalScroll is not missing value and originalScroll is not 1 then
				terminal's setScrollPosition(1)
				set didChangeScroll to true
			end if
			
			set shellPromptResult to false
			...
			if isSSH() and __getOutput() ends with "#" then
				...
				if isZsh() then
					...
					if promptText is missing value then
						set shellPromptResult to false
						
					else if promptText is equal to getNonGitPrefix() & "~" then
						set shellPromptResult to true
						...
						set shellPromptResult to promptGit or promptNonGit and promptText ends with directoryName
					end if
					...
					set shellPromptResult to isDocker and rtrimmedHistory ends with "#" or isSshShell or rtrimmedHistory ends with my promptEndChar or regex's matchesInString("bash-\\d(?:\\.\\d)?[#\\$]$", rtrimmedHistory)
				end if
			end if
			
			if didChangeScroll then terminal's setScrollPosition(originalScroll)
			shellPromptResult
		end isShellPrompt
